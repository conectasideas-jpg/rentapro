/**
 * Equipos.jsx — OPTIMIZADO
 * 1. useQuery('equipos') → caché de 1 min, segunda visita instantánea
 * 2. SkeletonStatCards + SkeletonEquiposGrid → no más spinner vacío
 * 3. invalidate('equipos') → solo recarga cuando hay cambios reales
 */
import { useState } from 'react'
import { supabase } from '../lib/supabase'
import { useQuery, invalidate } from '../lib/cache'
import { fetchEquipos } from '../lib/fetchers'
import { useAuth } from '../hooks/useAuth'
import PageHeader from '../components/PageHeader'
import Modal from '../components/Modal'
import { toast } from '../components/Toast'
import { SkeletonStatCards, SkeletonEquiposGrid } from '../components/Skeleton'
...
export default function Equipos() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <PageHeader title="Equipos" count={loading ? null : equipos.length}
        actions={isAdmin && (
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '18px 22px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 4 }}>
            <button className={`btn btn-sm ${view === 'cards' ? 'btn-primary' : ''}`} onClick={() => setView('cards')} title="Vista tarjetas">
              <i className="ti ti-layout-grid" />
            </button>
            <button className={`btn btn-sm ${view === 'table' ? 'btn-primary' : ''}`} onClick={() => setView('table')} title="Vista tabla">
              <i className="ti ti-table" />
            </button>
          </div>
        </div>

        {loading ? <SkeletonEquiposGrid count={6} /> : filtered.length === 0 ? (
          <div className="tcard">
            ...
            {filtered.map(e => {
              ...
              return (
                <div key={e.id} className="eq-card">
                  ...
                  <div className="eq-price">{fmt(e.precio_dia)}<span> /día</span></div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <span className={`badge b-dot ${lib > 0 ? 'b-avail' : 'b-rented'}`}>
                      {lib > 0 ? `${lib} disponible${lib > 1 ? 's' : ''}` : 'Sin stock'}
                    </span>
                    <span className="badge b-readonly" style={{ color: 'var(--text3)' }}>Stock: {e.stock}</span>
                  </div>
                  <div className="eq-card-footer">
                    {e.costo_compra > 0 && (
                      <div style={{ fontSize: 11, color: 'var(--text3)' }}>Rec. {Math.ceil(e.costo_compra / e.precio_dia)}d</div>
                    )}
                    {isAdmin && (
                      <div style={{ display: 'flex', gap: 4, marginLeft: 'auto' }}>
                        <button className="btn btn-sm btn-icon" title="Editar" onClick={() => openEdit(e)}><i className="ti ti-edit" /></button>
                        <button className="btn btn-sm btn-icon btn-danger" title="Eliminar" onClick={() => eliminar(e.id, e.nombre)}><i className="ti ti-trash" /></button>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  {filtered.map(e => {
                    ...
                    return (
                      <tr key={e.id}>
                        ...
                        <td style={{ textAlign: 'center' }}><span className={`badge b-dot ${lib > 0 ? 'b-avail' : 'b-rented'}`}>{lib}</span></td>
      ...
      {modal && (
        ...
        >
          <div className="form-grid">
            ...
            <div className="fg"><label>Precio / día ($) *</label><input type="number" value={form.precio_dia} onChange={e => f('precio_dia', e.target.value)} placeholder="0" /></div>
            <div className="fg"><label>Stock total</label><input type="number" min={1} value={form.stock} onChange={e => f('stock', e.target.value)} /></div>
            <div className="fg"><label>Costo de compra ($)</label><input type="number" value={form.costo_compra} onChange={e => f('costo_compra', e.target.value)} placeholder="0" /></div>
            <div className="fg"><label>Costo operacional / día ($)</label><input type="number" value={form.costo_operacional_dia} onChange={e => f('costo_operacional_dia', e.target.value)} placeholder="0" /></div>
          </div>
          {form.precio_dia > 0 && form.costo_compra > 0 && (
            <div className="calc-box" style={{ marginTop: 14 }}>
              <div className="calc-row"><span>Precio diario</span><span>{fmt(form.precio_dia)}</span></div>
              <div className="calc-row"><span>Costo de compra</span><span>{fmt(form.costo_compra)}</span></div>
